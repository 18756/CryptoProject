...
public class SubsetDifference {
    public static Set<Algo.DifferenceTrees> cover(List<byte[]> tree, Set<Integer> R) {
        ...
        Set<Algo.DifferenceTrees> CV = new HashSet<>();
        ...
        while (R.size() >= 2) {
            ...
            if (2 * lca + 1 != leftId) {
                CV.add(new Algo.DifferenceTrees(2 * lca + 1, leftId));
            }
            if (2 * lca + 2 != rightId) {
                CV.add(new Algo.DifferenceTrees(2 * lca + 2, rightId));
            }
        ...
        if (lastLeaf != 0) {
            CV.add(new Algo.DifferenceTrees(0, lastLeaf));
        }
        return CV;
    }
